...
#' 
#' 1 extra numeric variable, for sale price
#' 
...
#' 
#' Zoning codes; residential vs. commercial vs. agriculture
#' 
#' 2. Run the following two expressions.  Why do they give different results? 
#' 
#' First one filters out individual houses based on sale price; 
#' second one filters out zoning types based on mean sale price
#' 
dataf %>% 
    group_by(MS_Zoning) %>% 
...
#' 
#' Yes, same number of rows; so no duplicate entries
#' 
dataf_nodup = distinct(dataf)
identical(nrow(dataf), nrow(dataf_nodup))
...
#' 
#' Coercing cor_matrix to a dataframe (tibble), turning the row names into the `covar` variable
#' 

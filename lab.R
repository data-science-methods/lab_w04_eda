...
#' 
#' 
#' it seems like value, except that it says that is a tibble so I think that isa tibble which i think means its another data table of value stuff? makes zero sense to me how this is organized
#' 
...
#' 
#' Overall Qual, Lot Area. I would like one for value of neighboring houses but I dont see one.
#' 
...
skimr::skim(dataf)
#the skimr dunct gives a print out that says 293- is there a functional p[rogramming way of accessing that part of the returned tibble to set it as the answer?
...
#' 
#' 
#' for the number of factors it says there are 81 columns, I am assuming one of those is the index column and thus we have 80 variables. its hard to say though looking at skimr output. 
problem2.2.factors = 80
problem2.2.characters = 46
problem2.2.numerics = 35

#' 3. Explain any discrepancies here. 
#' 
#' There are 81 columns in our data and it says we have 46 plus 35 variables so trheres 81 asnd we shoud have 80, but I thihnk one is probably the index
#'
#' 
...
#' 
#' I dont follow the question? these are citiy district zoning labels, bu the description in the code book are almost exaclty the same as whats in the table, they arent acronyms or anything. Is there something im missing?
#' 
...
#in the first we filter before we zummarize and in the second we summarize first. so in the second were getting rid of the summaries that are below 100k and in the first we getting rid of everyone below 100k before we summarize so the averages could be higher in the first
...
#' 
#' 
#' they do. is there a way to do a tibble.shape?
# dataf_nodup = 2930
...
#' 
#' character, its a bunch of strings
#' 
...
# "Above_Average" "Average"       "Below_Average" "Excellent"     "Fair"          "Good"          "Poor"          "Very_Good"     "Very_Poor"

condition_levels = c("Very_Poor", "Poor", "Fair", "Below_Average", "Average", "Above_Average", "Good", "Very_Good", "Excellent")
...
dataf %>% 
    mutate(overall_cond_fct = good_factor)
...
#' 
#' 
#' it read the matreix as a tibble. like aslist or asarray. Is there something more menaingful im missing here?

cor_df = as_tibble(cor_matrix, rownames = 'covar')

#' b. What do the rows of `cor_df` represent?  The columns?  The values in each cell? 
#' - rows: variables
#' - columns: same variables
#' - values: correlation between row var and col var
...
#' 
#' Not good lol how ius quality not predictive trhats crazy haha
#' 

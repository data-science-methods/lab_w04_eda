...
#' A: I don't see the variable in the description, but in the data it should be Sale_Price
#' 
#' 
...
#' A: Neighborhood, Year Build, Overall Cond
#' 
#' 
...
#' A: Identifies the general zoning classification of the sale
#' 
#' 

#' 2. Run the following two expressions.  Why do they give different results? 
#' A: The first one calculates the average sale price of houses greater than 100000 for each zoning classification, the second one finds the zoning classification with average sale price greater than 100000
#' 
#' 
dataf %>% 
    group_by(MS_Zoning) %>% 
...
#' A: they have the same number of row, which means there is no duplicate entries in our data
#' 
#' 
...
#' A: It transforms a matrix into a dataframe.
#' 
#' 
...
#' - rows: the variables we selected
#' - columns: also the variables we selected
...
#' 
top_10 <- cor_df %>% select(covar,Sale_Price) %>% arrange(desc(Sale_Price)) %>% top_n(10)
